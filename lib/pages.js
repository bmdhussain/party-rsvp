...
const { isAdmin, authIsFresh } = require('./site-settings');
...
function registerPages(app, { pool, requirePageAuth, safeNext, makeShareUrl, makeImageUrl, findTemplate }) {
  ...
  app.get('/admin', requirePageAuth, (req, res) => {
    ...
    // An owner with a month-old session is sent back through Google before the
    // console will open, so a left-open browser is not a way in.
    if (!authIsFresh(req.session)) return res.redirect('/auth/google?reauth=1&next=/admin');
