...
// A first guess at where the host is, taken from the browser's own time zone.
//
// Deliberately not from the IP address: that would mean sending every visitor's
// address to a geolocation service, which is a fifth third party receiving
// data about people who have not asked for anything — on a site whose privacy
// page says there are no trackers. The time zone is already in the browser, it
// needs no permission prompt, and it tells us nothing we could not already see.
// IANA zone names carry a city, which is exactly what the forecast needs.
const ZONE_NOISE = /^(utc|gmt|universal|zulu|est|cst|mst|pst|est5edt|cst6cdt|mst7mdt|pst8pdt|w-su|rok|prc|roc|navajo|cuba|egypt|eire|iran|israel|japan|libya|nz|poland|portugal|singapore|turkey)$/i;

// Browsers still report retired zone names, and the retired name is often a
// city that has since been renamed — so it either finds nothing or, worse,
// finds a different place with the old spelling. Asia/Calcutta is the one that
// caught this: the geocoder has no Calcutta in India at all, and happily
// returns a town of 35,000 people in South Africa.
const ZONE_ALIASES = {
...
function canonicalZone(tz) {
  ...
  return ZONE_ALIASES[zone.toLowerCase()] || zone;
}

function townFromTimezone(tz) {
  const zone = canonicalZone(tz);
  // "Etc/GMT+5" and friends name an offset, not a place.
  if (!zone || !zone.includes('/') || zone.startsWith('Etc/')) return null;
  const city = zone.split('/').pop().replace(/_/g, ' ').trim();
  if (!city || ZONE_NOISE.test(city) || /^[+-]?\d/.test(city)) return null;
  return city;
}

// The suggestion itself. Offered, never applied on a host's behalf, because the
// town ends up on a page their guests read.
//
// A name alone is not enough to identify a place: there is a London in Ontario
// and a Calcutta in Mpumalanga. So a candidate only counts if its own time zone
// is the one the browser reported. If nothing matches, this returns nothing —
// a wrong town on an invitation is worse than no town at all.
async function suggestPlace(tz) {
  const zone = canonicalZone(tz);
  const city = townFromTimezone(zone);
  if (!city) return null;
  const matches = await searchPlaces(city);
  const sameZone = matches.filter((m) => m.timezone === zone);
  if (!sameZone.length) return null;
  return sameZone.reduce((best, m) => ((m.population || 0) > (best.population || 0) ? m : best));
}

// Turning a pair of coordinates back into a name, for the host who taps "use my
// location" rather than typing. The browser's geolocation gives coordinates
// accurate to metres — which is all the forecast actually needs — but a host
// should see a place name, not two numbers, before it goes on their event page.
//
// This runs only when a host asks for it. It is not a lookup done to every
// visitor who opens the site.
async function reverseGeocode(latitude, longitude) {
  ...
  if (!town) return null;
  return {
    label: [town, data.principalSubdivision !== town ? data.principalSubdivision : null, data.countryName]
      .filter(Boolean)
      .join(', '),
    // The coordinates the browser gave, not the ones the name resolves to: they
    // are more precise, and precision is the whole point of asking.
    latitude: lat,
    longitude: lon,
    timezone: null,
  };
...
async function searchPlaces(query) {
  ...
  // Biggest first: someone typing "London" almost always means the one with
  // nine million people in it, not the one in Ohio.
  return (data.results || [])
    .map((r) => ({
...
module.exports = {
  ...
  townFromTimezone, suggestPlace, canonicalZone, reverseGeocode,
