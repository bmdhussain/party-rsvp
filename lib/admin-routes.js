...
function registerAdminRoutes(app, { pool, verifySameOrigin }) {
  function requireAdmin(req, res, next) {
    ...
    if (!siteSettings.authIsFresh(req.session)) {
      // Being signed in is not enough here. The browser may have been left open,
      // or handed over; the console asks the identity provider to check again.
      return res.status(401).json({ error: 'Confirm it is you again.', reauth: '/auth/google?reauth=1&next=/admin' });
    }
    next();
  }
