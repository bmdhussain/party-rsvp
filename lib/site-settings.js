...
// How recently the identity provider must have actually checked who this is
// before the console will open. A signed-in session lasts thirty days; that is
// the right length for hosting a party and the wrong length for changing the
// whole site, so the console asks Google to prove it again.
const ADMIN_REAUTH_MAX_AGE_MS = Number(process.env.ADMIN_REAUTH_MINUTES || 15) * 60 * 1000;

function authIsFresh(session) {
  const at = session && session.authTime;
  // No recorded authentication time means we cannot say it is fresh, so it is
  // not. A session predating this check is treated as stale, once.
  if (!at) return false;
  return Date.now() - at < ADMIN_REAUTH_MAX_AGE_MS;
}
...
module.exports = {
  ...
  isAdmin, adminEmails, authIsFresh, ADMIN_REAUTH_MAX_AGE_MS,
};
