...
const DEFAULT_DESCRIPTION =
  'Beautiful invitations and effortless RSVPs for birthdays, dinners, weddings and every gathering in between.';
...
function renderHead({
  ...
  const fullTitle = title ? `${title} · ${SITE_NAME}` : `${SITE_NAME} — Invitations worth opening`;
...
function siteHeader({ user, active = '' } = {}) {
  ...
  return `<header class="site-header app-header">
      ...
      <nav class="header-nav primary-nav" aria-label="Main">
        ...
        <a class="btn btn-primary btn-small nav-cta" href="/events/new">+ Create event</a>
        ${account}
      </nav>
    </header>`;
}
...
function siteFooter({ user } = {}) {
  return `<footer class="site-footer app-footer">
      <div class="footer-brand"><span class="footer-wordmark"><span class="brand-rsvp">RSVP</span><span class="brand-for">for</span></span><small>Invitations worth opening.</small></div>
  ...
  return `<div class="workspace-head">
      ...
      <div class="workspace-title-row">
        ...
        <div class="workspace-actions">
          <a class="btn btn-ghost btn-small" href="/e/${escapeHtml(event.slug)}" target="_blank" rel="noopener">${
            live ? 'View guest page ↗' : 'Preview ↗'
          }</a>
          ${actions ? '<button class="btn btn-primary btn-small" id="copy-link-btn" type="button">Copy invite link</button>' : ''}
        </div>
      </div>
        ...
        <div class="workspace-actions">
          ${
            isRsvp
              ...
              : `<a class="btn btn-ghost btn-small" href="/f/${escapeHtml(form.slug)}" target="_blank" rel="noopener">${
