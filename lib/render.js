...
const { findDoodle } = require('./site-settings');
...
function renderHead({
  ...
  const tags = [
    ...
    // After app.css, because a theme's whole job is to win against the default
    // palette. Loaded here rather than in each template so a new theme needs no
    // change to twenty HTML files.
...
// The logo is either the wordmark or an image the owner uploaded, and either
// may carry a seasonal doodle. The doodle is drawn inline so it takes the
// header's own colours and stays sharp when the page is zoomed or printed.
function brandMark(settings = {}) {
  const doodle = findDoodle(settings.doodle);
  const mark = doodle && doodle.svg
    ? `<span class="brand-doodle" aria-hidden="true"><svg viewBox="0 0 24 24" width="26" height="26">${doodle.svg}</svg></span>`
    : '';
  const inner = settings.logo_mime
    ? `<img class="brand-image" src="/site/logo?v=${encodeURIComponent(
        settings.logo_updated_at ? new Date(settings.logo_updated_at).getTime() : '0'
      )}" alt="${escapeHtml(SITE_NAME)}" />`
    : `<span class="brand-mark">R</span><span class="brand-wordmark"><span class="brand-rsvp">RSVP</span><span class="brand-for">for</span></span>`;
  return `<a class="brand${doodle && doodle.svg ? ' has-doodle' : ''}" href="/" aria-label="${escapeHtml(
    SITE_NAME
  )} home">${inner}${mark}</a>`;
}
...
function siteHeader({ user, active = '', settings = {}, admin = false } = {}) {
  ...
  const account = user
    ? `<details class="account-menu">
        ...
        <div class="account-menu-panel">
          ...
          ${admin ? '<a href="/admin">Site admin</a>' : ''}
...
function siteFooter({ user, settings = {} } = {}) {
  return `<footer class="site-footer app-footer">
      <div class="footer-brand">${
        settings.logo_mime
          ? `<img class="footer-logo" src="/site/logo" alt="${escapeHtml(SITE_NAME)}" />`
          : '<span class="footer-wordmark"><span class="brand-rsvp">RSVP</span><span class="brand-for">for</span></span>'
      }<small>Invitations and events worth showing up for.</small></div>
...
function renderPage(res, file, { head = '', header = '', footer = '', workspace = '', slots = {}, status = 200, theme = '' } = {}) {
  ...
  // The stylesheets hang their alternate palettes off this attribute, so the
  // theme has to be on the document before the first paint — not applied by
  // script afterwards, which would flash the default theme first.
  if (theme && theme !== 'default' && /^[a-z0-9-]{1,32}$/.test(theme)) {
